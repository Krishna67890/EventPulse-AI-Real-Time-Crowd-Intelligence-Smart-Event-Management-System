...
import { generateSuggestions, speakIntroduction, triggerEmergencyAlert } from '../utils/aiLogic';

import { Link, useNavigate } from 'react-router-dom';
import { requestLocationPermission, requestNotificationPermission } from '../utils/permissionUtils';
...
const DashboardPage = () => {
  ...
  useEffect(() => {
    ...
    // Simulation of periodic emergency events
    const emergencyTimer = setInterval(() => {
      if (Math.random() > 0.9) {
          setIsEmergencyMode(true);
          setTimeout(() => setIsEmergencyMode(false), 8000);
      }
    }, 30000);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Use environment variable or relative path if deployed on same host
        const API_BASE = window.location.hostname === 'localhost'
          ? 'http://localhost:5000'
          : 'https://your-backend-url.vercel.app'; // Update this after deploying backend
        ...
        if (!lowNetworkMode) console.warn("Syncing with Local Neural Cache (Fallback Data)");
        setLowNetworkMode(true);
        // Fallback dummy data if backend fails
        // Advanced Multi-Node Global Network (Expanded)
        const dummyZones = [
          { id: 'gate_a', name: "Alpha Sector (Gate A)", type: 'CROWD', lat: 51.508, lng: -0.085, people: 1450, capacity: 1500, waitTime: 18, comfortScore: 35, ai: { current: { color: '#ff0000' } } },
          { id: 'main_stage', name: "Neural Core (Main Stage)", type: 'CROWD', lat: 51.51, lng: -0.09, people: 4950, capacity: 5000, waitTime: 55, comfortScore: 10, ai: { current: { color: '#ff0000' } } },
          { id: 'airport_link', name: "Aero-Transit Hub", type: 'AIRPORT', lat: 51.518, lng: -0.1, people: 2800, capacity: 3000, waitTime: 40, comfortScore: 40, ai: { current: { color: '#00ff00' } } },
          { id: 'hospital_city', name: "Global Medical Center", type: 'MEDICAL', lat: 51.514, lng: -0.075, people: 300, capacity: 1000, waitTime: 10, comfortScore: 90, ai: { current: { color: '#00f0ff' } } },
          { id: 'food_court_1', name: "Sustenance Hub A", type: 'RESTAURANT', lat: 51.505, lng: -0.08, people: 850, capacity: 1000, waitTime: 25, comfortScore: 45, ai: { current: { color: '#0000ff' } } },
          { id: 'food_court_2', name: "Sustenance Hub B", type: 'RESTAURANT', lat: 51.503, lng: -0.092, people: 920, capacity: 1000, waitTime: 30, comfortScore: 30, ai: { current: { color: '#0000ff' } } },
          { id: 'washroom_north', name: "North Sanitation", type: 'WASHROOM', lat: 51.512, lng: -0.088, people: 180, capacity: 200, waitTime: 12, comfortScore: 40, ai: { current: { color: '#ffcc00' } } },
          { id: 'train_station_1', name: "Neural Rail Base", type: 'TRAIN_STATION', lat: 51.5, lng: -0.085, people: 195, capacity: 500, waitTime: 15, comfortScore: 70, ai: { current: { color: '#ffff00' } } },
          { id: 'vip_lounge', name: "Zenith Lounge (VIP)", type: 'VIP', lat: 51.509, lng: -0.095, people: 120, capacity: 500, waitTime: 2, comfortScore: 98, ai: { current: { color: '#00f0ff' } } },
          { id: 'exit_east', name: "Eastern Gateway", type: 'EXIT', lat: 51.507, lng: -0.075, people: 400, capacity: 2000, waitTime: 1, comfortScore: 95, ai: { current: { color: '#00f0ff' } } },
          { id: 'medical_tent_1', name: "Field Med-Link 01", type: 'MEDICAL', lat: 51.511, lng: -0.082, people: 85, capacity: 100, waitLine: 25, comfortScore: 40, ai: { current: { color: '#ffcc00' } } },
          { id: 'wifi_zone_1', name: "Data Uplink Alpha", type: 'WIFI', lat: 51.506, lng: -0.089, people: 980, capacity: 1000, waitTime: 0, comfortScore: 15, ai: { current: { color: '#ff0000' } } },
          { id: 'charging_hub', name: "Power Grid Base", type: 'POWER', lat: 51.513, lng: -0.081, people: 380, capacity: 400, waitTime: 35, comfortScore: 25, ai: { current: { color: '#ff0000' } } },
          { id: 'merch_stand_alpha', name: "Asset Distribution A", type: 'RESTAURANT', lat: 51.504, lng: -0.084, people: 750, capacity: 800, waitTime: 28, comfortScore: 35, ai: { current: { color: '#0000ff' } } },
          { id: 'merch_stand_beta', name: "Asset Distribution B", type: 'RESTAURANT', lat: 51.516, lng: -0.087, people: 100, capacity: 800, waitTime: 2, comfortScore: 95, ai: { current: { color: '#00f0ff' } } },
          { id: 'entry_beta', name: "Beta Transit Link", type: 'EXIT', lat: 51.515, lng: -0.092, people: 200, capacity: 2000, waitTime: 2, comfortScore: 92, ai: { current: { color: '#00f0ff' } } },
          { id: 'security_post_4', name: "Guardian Post 04", type: 'SECURITY', lat: 51.502, lng: -0.086, people: 120, capacity: 300, waitTime: 4, comfortScore: 88, ai: { current: { color: '#00f0ff' } } },
          { id: 'train_station_west', name: "Central Station (West)", type: 'TRAIN_STATION', lat: 51.52, lng: -0.08, people: 8500, capacity: 10000, waitTime: 20, comfortScore: 60, ai: { current: { color: '#ffff00' } } }
        ];
        setCrowdData(dummyZones);
        setSuggestions(generateSuggestions(dummyZones));
  ...
  useEffect(() => {
    if (isEmergencyMode) {
        ...
        triggerEmergencyAlert(selectedZone?.name || "Neural Core");
    } else {
        document.body.classList.remove('emergency-mode');
        // Do not cancel introductory speech if it was running
    }
  }, [isEmergencyMode]);
      ...
      <nav className="fixed bottom-0 left-0 w-full h-16 border-t lg:border-t-0 lg:border-r border-white/5 flex lg:flex-col items-center justify-around lg:justify-start lg:py-10 lg:w-24 lg:h-full z-[1000] bg-black/40 backdrop-blur-xl">
        ...
        <div className="flex lg:flex-col gap-8 lg:gap-10">
            ...
            <Link to="/profile" className="text-white/40 hover:text-neonBlue transition-colors cursor-pointer group flex flex-col items-center">
                <User size={22} />
                <span className="text-[7px] font-black uppercase mt-1 tracking-widest lg:opacity-0 group-hover:opacity-100 transition-opacity truncate max-w-[50px]">
                    {localStorage.getItem('user_name') || (localStorage.getItem('guest_id') ? 'GUEST' : 'Profile')}
                </span>
            </Link>
